...
fn index_test() {
    ...
    assert_eq!(index_usize, Ok(2..4));
}

#[test]
fn index_almost_ov_begin_test() {
    let index =
        MCUIndex::<T>::new(usize::MAX / 2)..MCUIndex::<T>::new(usize::MAX / 2);
    let index_usize = MCURange::from(index).try_into();
...
#[test]
fn index_ov_end_test() {
    let index = MCUIndex::<T>::new(1)..MCUIndex::<T>::new(1 + (usize::MAX / 2));
    let index_usize: Result<core::ops::Range<_>, MCUIndexByteOverflow> =
        MCURange::from(index).try_into();
    assert_eq!(index_usize, Err(MCUIndexByteOverflow));
}
...
macro_rules! test {
        ($($range:tt[$lb:literal .. $ub:literal] == $res:tt,)+) => {
            $(
                ...
                let index = MCUIndex::<T>::new($lb)..MCUIndex::<T>::new($ub);
